...
interface Customer {
  ...
  countryCode: string; // Add country code (e.g., "US", "GB", "DE")
}

const OurCustomers = () => {
  const [activeTab, setActiveTab] = useState<'Medical' | 'E-commerce' | 'POS'>('Medical');
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4">
        <h2 className="text-3xl font-extrabold text-gray-900 dark:text-gray-100 text-center mb-8">Our Customers</h2>

        {/* Tabs */}
        <div className="flex flex-wrap justify-center mb-8">
          {['Medical', 'E-commerce', 'POS'].map((tab) => (
            <motion.button
              key={tab}
              className={`px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 rounded-full mr-2 mb-2 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500
                                ${activeTab === tab ? 'bg-blue-500 text-white shadow-md' : 'bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600'}`}
              onClick={() => setActiveTab(tab as 'Medical' | 'E-commerce' | 'POS')}
        ...
        <div className="relative">
          <button
            className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-75 hover:bg-opacity-100 text-gray-700 dark:text-gray-300 p-2 rounded-full shadow focus:outline-none"
            onClick={scrollLeft}
          >
            <IoIosArrowBack size={24} />
          </button>
          <button
            className="absolute right-0 top-1/2 transform -translate-y-1/2 bg-white bg-opacity-75 hover:bg-opacity-100 text-gray-700 dark:text-gray-300 p-2 rounded-full shadow focus:outline-none"
            onClick={scrollRight}
          >
            <IoIosArrowForward size={24} />
          </button>
          <div
            className="flex overflow-x-auto space-x-6 snap-x snap-mandatory scroll-smooth py-4"
            ref={containerRef}
          >
            <AnimatePresence>
              {filteredCustomers.map((customer) => (
                <motion.div
                  key={customer.id}
                  className="bg-white dark:bg-gray-800 rounded-lg shadow-md overflow-hidden snap-start shrink-0 w-64 sm:w-72 md:w-80"
                ...
                >
                  <Image
                    ...
                    priority={filteredCustomers.indexOf(customer) < 2}
                  />
                  <div className="p-4">
                    <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100">
                      {customer.name}
                      <Image
                        src={`/flags/${customer.countryCode.toLowerCase()}.svg`} // Assuming your flag images are in public/flags
                        alt={`${customer.countryCode} Flag`}
                        width={24}  // Adjust as needed
                        height={16} // Adjust as needed
                        style={{
                          display: 'inline-block',
                          verticalAlign: 'middle', // Adjust for best alignment
                          marginLeft: '5px',        // Add some space between name and flag
                          objectFit: 'contain',
                        }}
                      />
                    </h3>
